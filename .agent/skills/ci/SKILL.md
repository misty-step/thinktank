...
## Repo-specific gate (thinktank)

In this repo the gate is a single command: `./scripts/with-colima.sh dagger
call check`. It enforces format, compile-warnings-as-errors, credo --strict,
Dialyzer, shell/YAML hygiene, gitleaks, a repo-owned security gate, live
OpenRouter model-ID validation, an architecture gate, escript smoke, and an
**87% coverage floor** (not 80%). The native pre-push hook runs the same
command. Host-native `mix` commands are debugging fallback, not the gate.
Read `references/thinktank-gate.md` before auditing or running — it lists
the enforced checks, required env (`THINKTANK_OPENROUTER_API_KEY`, Colima
running), and the red-line invariants that must not be lowered.
