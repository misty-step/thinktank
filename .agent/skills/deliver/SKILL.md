...
Base branch is **`master`**, not `main`. The clean-loop gate is
`./scripts/with-colima.sh dagger call check` (see
`references/thinktank-gate.md`). Backlog source is `backlog.d/` (numbered
markdown files; closed items move to `backlog.d/done/` via `git mv`).
`/qa` here is CLI verification, not browser QA — stay on `--dry-run`
unless the diff touches executor, prompts, or synthesis. Release-please
owns version bumps; /deliver never edits `@version` in `mix.exs`.
