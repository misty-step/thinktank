...
Primary branch is **`master`**. Gate is `./scripts/with-colima.sh dagger
call check` — same as /ci and the pre-push hook. Default land policy is
squash-on-land (one-ticket-per-branch discipline via `backlog.d/`).
**release-please** owns versioning: do not edit `@version` in `mix.exs`,
`CHANGELOG.md`, or cut tags manually. Write Conventional Commit messages
and let release-please open the release PR. See
`references/thinktank-settle.md` for the full landing checklist, CODEOWNERS
notes, and hook-bypass red lines.
