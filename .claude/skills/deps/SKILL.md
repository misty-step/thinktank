...
Ecosystem is **Elixir/Hex** (`mix.exs`, `mix.lock`). Runtime deps are tiny
and load-bearing: `jason` (all JSON artifacts), `yaml_elixir` (config),
`muontrap` (Pi subprocess launcher — executor core). A bad `muontrap`
upgrade breaks every agent launch; smoke a real research run after
bumping it, not just `--dry-run`. Dev-only: `credo`, `dialyxir`,
`excoveralls`. Use `chore(deps):` commits so release-please classifies
them correctly. See `references/thinktank-deps.md` for reachability
analysis, PLT warm-up caveats, and the "no unused deps" rule.
