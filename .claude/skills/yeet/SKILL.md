...
Primary branch is **`master`**. The pre-push hook at `.githooks/pre-push`
runs `./scripts/with-colima.sh dagger call check`; a hook failure is a
real signal, never pass `--no-verify`. Conventional Commits are
load-bearing because **release-please** parses them to cut releases — use
`feat:`/`fix:`/`chore:`/`refactor:`/etc., scope is optional, breaking
changes via `!`. Stage files explicitly by path: the repo has stray local
state (`.env`, `erl_crash.dump`, `thinktank.log`, worktree dirs like
`silver-spinning-quartz/`) that `git add -A` will wrongly pick up. See
`references/thinktank-yeet.md` for the full don't-commit list and
architecture-gate red lines.
