defmodule Thinktank.Router do
    ...
    "properties" => %{
      "perspectives" => %{
        ...
        "items" => %{
          "type" => "object",
          "properties" => %{
            "role" => %{"type" => "string", "description" => "Expert role name (e.g. 'security auditor')"},
            "model" => %{"type" => "string", "description" => "Model ID from the available_models list"},
            "system_prompt" => %{
              "type" => "string",
  ...
  Options:
    - `:available_models` — list of model IDs the router may assign
  ...
  Models are round-robined across roles.
  """
  ...
  def manual_perspectives(roles, available_models) do
    ...
    |> Enum.with_index()
    |> Enum.map(fn {role, i} ->
      model = Enum.at(available_models, rem(i, length(available_models)))
